...
class Impulso(object):
    ...
    def dataset(self):
        ...
        for dtype in ['train', 'test']:
            if dtype == 'test' and os.path.exists(os.path.join(IMPULSO_HOME, 'datasets', 'test')):
                logger.info('Test data exists. Skip creating test data.')
                continue
            aggregator.load_data(dtype)
            aggregator.save_data(dtype)
